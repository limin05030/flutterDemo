...
class BottomTabBar extends StatefulWidget {
  ...
  /// track of the index of the selected [BottomTabBarItem] and call
  /// `setState` to rebuild the bottom navigation bar with the new [currentIndex].
  final ValueChanged<int> onTap;

  /// The index into [items] for the current active [BottomTabBarItem].
  ...
  // Used by the [BottomTabBar] constructor to set the [type] parameter.
  //
  // If type is provided, it is returned. Otherwise,
  // [BottomTabBarType.fixed] is used for 3 or fewer items, and
  // [BottomTabBarType.shifting] is used for 4+ items.
  static BottomTabBarType _type(
    ...
    return items.length <= 3 ? BottomTabBarType.fixed : BottomTabBarType.shifting;
  }

  // Used by the [BottomTabBar] constructor to set the [showUnselected]
  // parameter.
  //
  // Unselected labels are shown by default for [BottomTabBarType.fixed],
  // and hidden by default for [BottomTabBarType.shifting].
...
class _BottomNavigationTile extends StatelessWidget {
        ...
        child: Focus(
          child: Stack(
            children: <Widget>[
              ...
              Positioned(right: 14, top: 2, child: _buildBadge()),
              Semantics(
                label: indexLabel,
              ),
            ],
  ...
  Widget _buildBadge() {
    if (item.badgeNo == null || item.badgeNo <= 0) {
      return Container();
    }

    String text = item.badgeNo < 100 ? "${item.badgeNo}" : "99+";
...
class _BottomNavigationBarState extends State<BottomTabBar> with TickerProviderStateMixin {
